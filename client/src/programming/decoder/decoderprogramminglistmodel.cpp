...
DecoderProgrammingListModel::DecoderProgrammingListModel(TableModelPtr tableModel, QObject* parent)
...
int DecoderProgrammingListModel::rowCount(const QModelIndex& parent) const
{
  return 1 + m_tableModel->rowCount(parent);
}

QVariant DecoderProgrammingListModel::data(const QModelIndex& index, int role) const
{
  if(index.row() == 0)
